...
Token names use CSS custom property syntax (`--md-sys-color-*`). Never use raw hex values — always reference by token name.
...
| `--md-sys-color-dark-secondary` | `#252A36` | Cards, list items |
| `--md-sys-color-dark-tertiary` | `#3D4451` | Dividers, borders |
...
| Token | Value | Use |
|---|---|---|
| `--md-sys-color-recording` | `#FF5A4F` | Live recording indicator (distinct from error red) |
...
3. Alpha/glass effects use `--md-sys-color-alpha-*` tokens + `backdrop-filter: blur()`.
