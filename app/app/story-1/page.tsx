  ...
  return (
    <section id={id} style={{ marginBottom: 72 }}>
      <h2 style={{
        ...
        color: "var(--md-sys-color-neonindigo)", marginBottom: 24, paddingBottom: 12,
...
function StatusBadge({ status }: { status: "built" | "decision" | "design" }) {
  const map = {
    ...
    design:   { label: "Needs design",    bg: "rgba(139,146,255,0.12)",  color: "var(--md-sys-color-neonindigo)" },
  };
  const s = map[status];
  return (
...
function ProtoLink({ path, label }: { path: string; label: string }) {
  return (
    <a href={protoLink(path)} target="_blank" rel="noopener noreferrer" style={{
      fontSize: 12, fontWeight: 600, color: "var(--md-sys-color-neonindigo)", textDecoration: "none",
...
function MockScreen({ children, width = 300, label }: { children: React.ReactNode; width?: number; label?: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "flex-start" }}>
      ...
      <div style={{
        width, background: "var(--md-sys-color-background)", borderRadius: 16,
        border: "1px solid var(--md-sys-color-dark-tertiary)", overflow: "hidden",
      }}>
        {children}
      </div>
    </div>
...
function MockHeader({ title }: { title: string }) {
  return (
    <div style={{ padding: "16px 16px 10px", borderBottom: "1px solid var(--md-sys-color-dark-tertiary)" }}>
      <p style={{ fontSize: 20, fontWeight: 700, color: "var(--md-sys-color-text-primary)", margin: 0, fontFamily: "Roboto Slab, Georgia, serif" }}>
        {title}
      </p>
    </div>
  );
  ...
  return (
    <div style={{ padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{
        ...
        borderRadius: 999, background: "var(--md-sys-color-dark-secondary)",
        outline: active ? "1.5px solid var(--md-sys-color-neonindigo)" : "none",
      }}>
        <svg width="15" height="15" viewBox="0 0 18 18" fill="none" style={{ flexShrink: 0 }}>
          <circle cx="7.5" cy="7.5" r="6" stroke={active ? "var(--md-sys-color-neonindigo)" : "var(--md-sys-color-text-muted)"} strokeWidth="1.75" />
          <path d="M12 12L16 16" stroke={active ? "var(--md-sys-color-neonindigo)" : "var(--md-sys-color-text-muted)"} strokeWidth="1.75" strokeLinecap="round" />
        </svg>
        <span style={{ fontSize: 14, color: query ? "var(--md-sys-color-text-primary)" : "var(--md-sys-color-text-disabled)", flex: 1 }}>
          {query || placeholder}
        </span>
      </div>
      {pill && (
        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.05em", color: "var(--md-sys-color-neonindigo)", background: "rgba(139,146,255,0.12)", borderRadius: 6, padding: "2px 7px", flexShrink: 0 }}>
          {pill}
        </span>
      )}
    </div>
...
function MockCard({ name, meta, type, owned = true, sync = "synced", lastVisited, isLast, restricted, dim }: MockCardProps) {
  ...
  return (
    <div style={{
      padding: "11px 14px",
      borderBottom: isLast ? "none" : "1px solid var(--md-sys-color-dark-tertiary)",
      opacity: dim ? 0.45 : 1,
      background: restricted ? "rgba(255,107,90,0.03)" : undefined,
    }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: restricted ? "var(--md-sys-color-text-muted)" : "var(--md-sys-color-text-primary)", margin: "0 0 3px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            {name}
          </p>
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <span style={{
              ...
              color: isAccount ? "var(--md-sys-color-neonindigo)" : "#F5A623",
            }}>
              <span style={{ width: 5, height: 5, borderRadius: "50%", background: isAccount ? "var(--md-sys-color-neonindigo)" : "#F5A623", flexShrink: 0 }} />
              {isAccount ? "Account" : "Lead"}
            </span>

            {/* Ownership */}
            {!owned && (
              <span style={{ padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: "rgba(255,143,130,0.1)", color: "var(--md-sys-color-brand-coral-light)" }}>
                Not yours
              </span>
            )}
            {owned && type === "account" && (
              <span style={{ padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 500, background: "var(--md-sys-color-dark-secondary)", color: "var(--md-sys-color-text-disabled)" }}>
                Yours
              </span>
            )}
            ...
            {sync === "needs-details" && <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: "rgba(255,107,90,0.1)", color: "var(--md-sys-color-brand-coral)" }}><span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--md-sys-color-brand-coral)" }} />Needs details</span>}
            ...
            {/* Last visited */}
            {lastVisited && <span style={{ fontSize: 10, color: "var(--md-sys-color-text-disabled)", marginLeft: 2 }}>Visited {lastVisited}</span>}
          </div>
        </div>
        ...
        {!restricted ? (
          ...
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, marginTop: 3 }}>
            <rect x="2" y="6" width="10" height="7" rx="1.5" stroke="var(--md-sys-color-text-disabled)" strokeWidth="1.25"/>
            <path d="M4.5 6V4.5C4.5 3.12 5.62 2 7 2C8.38 2 9.5 3.12 9.5 4.5V6" stroke="var(--md-sys-color-text-disabled)" strokeWidth="1.25" strokeLinecap="round"/>
          </svg>
        )}
      </div>
      {restricted && (
        <p style={{ fontSize: 10, color: "var(--md-sys-color-text-disabled)", margin: "5px 0 0" }}>
  ...
  return (
    <div style={{ padding: "0 12px 10px" }}>
      <div style={{
        ...
        border: `1px solid ${unavailable ? "var(--md-sys-color-dark-tertiary)" : "var(--md-sys-color-dark-tertiary)"}`,
        opacity: unavailable ? 0.5 : 1,
      }}>
        <svg width="14" height="14" viewBox="0 0 18 18" fill="none">
          <circle cx="9" cy="9" r="7" stroke={unavailable ? "var(--md-sys-color-text-disabled)" : "var(--md-sys-color-text-secondary)"} strokeWidth="1.5"/>
          <path d="M9 5V9L11.5 11.5" stroke={unavailable ? "var(--md-sys-color-text-disabled)" : "var(--md-sys-color-text-secondary)"} strokeWidth="1.5" strokeLinecap="round"/>
        </svg>
        <span style={{ fontSize: 13, fontWeight: 600, color: unavailable ? "var(--md-sys-color-text-disabled)" : "var(--md-sys-color-text-primary)" }}>
          {unavailable ? "Global search unavailable" : "Search all companies"}
        </span>
      </div>
      {unavailable && (
        <p style={{ fontSize: 10, color: "var(--md-sys-color-text-disabled)", textAlign: "center", margin: "6px 0 0" }}>
  ...
  return (
    <div style={{ padding: "11px 14px", borderBottom: isLast ? "none" : "1px solid var(--md-sys-color-dark-tertiary)" }}>
      ...
      <div style={{ display: "flex", gap: 5 }}>
        ...
        <div style={{ height: 18, width: 40, borderRadius: 20, background: "var(--md-sys-color-dark-secondary)" }} />
      </div>
    </div>
  );
}
...
function MockEmptyState({ label }: { label: string }) {
  return (
    <div style={{ padding: "32px 16px", textAlign: "center" }}>
      ...
      <p style={{ fontSize: 13, fontWeight: 600, color: "var(--md-sys-color-text-primary)", margin: "0 0 4px" }}>No results</p>
      <p style={{ fontSize: 12, color: "var(--md-sys-color-text-muted)", margin: 0 }}>{label}</p>
    </div>
  );
}
...
function OptionCard({
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${recommended ? "rgba(139,146,255,0.4)" : "var(--md-sys-color-dark-tertiary)"}`,
      borderRadius: 12,
      overflow: "hidden",
      flex: 1,
      minWidth: 0,
      ...
      <div style={{ padding: "14px 16px 12px", borderBottom: children ? "1px solid var(--md-sys-color-dark-tertiary)" : "none" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
          ...
          {recommended && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: "1px 7px", borderRadius: 20, background: "rgba(139,146,255,0.15)", color: "var(--md-sys-color-neonindigo)" }}>
              Recommended
            </span>
          )}
        </div>
...
const CHIPS = [
  ...
  { label: "Yours",            dot: undefined, bg: "var(--md-sys-color-dark-secondary)", color: "var(--md-sys-color-text-disabled)", usage: "Assigned to the logged-in rep" },
    ...
    <div style={{ minHeight: "100vh", background: "var(--md-sys-color-background)", color: "var(--md-sys-color-text-primary)", fontFamily: "Barlow, system-ui, sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: "var(--md-sys-color-text-primary)" }}>Story 1</span>
          <span style={{ color: "var(--md-sys-color-dark-tertiary)" }}>/</span>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <a href={protoLink("/relationships")} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, fontWeight: 600, color: "var(--md-sys-color-neonindigo)", textDecoration: "none", padding: "4px 12px", background: "rgba(139,146,255,0.1)", borderRadius: 20 }}>
            Open prototype ↗
          </a>
        </div>
      </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ background: "var(--md-sys-color-dark-primary)", border: "1px solid var(--md-sys-color-dark-tertiary)", borderRadius: 12, padding: "24px 28px", marginBottom: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                {[
                  ...
                  { label: "Needs design",      value: "9", color: "var(--md-sys-color-neonindigo)", bg: "rgba(139,146,255,0.1)" },
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div style={{ background: "var(--md-sys-color-dark-primary)", border: "1px solid var(--md-sys-color-dark-tertiary)", borderRadius: 10, padding: "16px 18px" }}>
                ...
                {[
                  ...
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "5px 0", borderBottom: i < 5 ? "1px solid var(--md-sys-color-dark-tertiary)" : "none" }}>
          ...
          <Section id="decisions" title="Key Decisions">
            ...
            <div style={{ marginBottom: 40 }}>
              <div style={{ marginBottom: 12 }}>
                ...
                <p style={{ fontSize: 13, color: "var(--md-sys-color-text-muted)", lineHeight: 1.6, margin: 0 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                ...
                >
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    <div style={{ height: 32, display: "flex", alignItems: "center", background: "var(--md-sys-color-dark-secondary)", borderRadius: 8, padding: "0 12px" }}>
                      <span style={{ fontSize: 13, color: "var(--md-sys-color-text-primary)", fontFamily: "Roboto Slab, Georgia, serif", fontWeight: 700 }}>Customers</span>
                    </div>
                    <div style={{ height: 32, display: "flex", alignItems: "center", background: "var(--md-sys-color-dark-secondary)", borderRadius: 8, padding: "0 12px" }}>
                      <span style={{ fontSize: 12, color: "var(--md-sys-color-text-disabled)" }}>Search customers…</span>
                    </div>
                  </div>
                </OptionCard>
                <OptionCard
                ...
                >
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    <div style={{ height: 32, display: "flex", alignItems: "center", background: "var(--md-sys-color-dark-secondary)", borderRadius: 8, padding: "0 12px" }}>
                      <span style={{ fontSize: 13, color: "var(--md-sys-color-text-primary)", fontFamily: "Roboto Slab, Georgia, serif", fontWeight: 700 }}>Companies</span>
                    </div>
                    <div style={{ height: 32, display: "flex", alignItems: "center", background: "var(--md-sys-color-dark-secondary)", borderRadius: 8, padding: "0 12px" }}>
                      <span style={{ fontSize: 12, color: "var(--md-sys-color-text-disabled)" }}>Search companies…</span>
                    </div>
                  </div>
                </OptionCard>
                <OptionCard
            ...
            <div style={{ marginBottom: 40 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                >
                  <MockScreen label="" width={268}>
                    {/* Fake tabs */}
                    <div style={{ display: "flex", borderBottom: "1px solid var(--md-sys-color-dark-tertiary)" }}>
                      {["Accounts", "Leads"].map((t, i) => (
                        <div key={t} style={{
                          flex: 1, textAlign: "center", padding: "10px 0", fontSize: 13, fontWeight: i === 0 ? 700 : 400,
                          color: i === 0 ? "var(--md-sys-color-text-primary)" : "var(--md-sys-color-text-muted)",
                          borderBottom: i === 0 ? "2px solid var(--md-sys-color-neonindigo)" : "2px solid transparent",
                        }}>
                          {t}
                        </div>
                      ))}
                ...
                >
                  <MockScreen width={268}>
                    ...
                    <div style={{ padding: "0 14px 12px" }}>
                      <p style={{ fontSize: 11, color: "var(--md-sys-color-text-disabled)", textAlign: "center", margin: 0 }}>
            ...
            <div style={{ marginBottom: 0 }}>
              ...
              <div style={{ background: "var(--md-sys-color-dark-primary)", border: "1px solid var(--md-sys-color-dark-tertiary)", borderRadius: 10, overflow: "hidden" }}>
                {[
                  ...
                  <div key={row.field} style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 1.2fr 2fr", gap: 0, borderBottom: i < arr.length - 1 ? "1px solid var(--md-sys-color-dark-tertiary)" : "none" }}>
                    ...
                    <div style={{ padding: "10px 12px", fontSize: 12, color: "var(--md-sys-color-text-muted)", borderRight: "1px solid var(--md-sys-color-dark-tertiary)" }}>{row.account}</div>
                    <div style={{ padding: "10px 12px", fontSize: 12, color: "var(--md-sys-color-text-muted)", borderRight: "1px solid var(--md-sys-color-dark-tertiary)" }}>{row.lead}</div>
                    <div style={{ padding: "10px 12px", fontSize: 11, color: "var(--md-sys-color-text-disabled)", fontStyle: "italic" }}>{row.note}</div>
                  </div>
                ))}
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 1.2fr 2fr", gap: 0, background: "var(--md-sys-color-dark-secondary)" }}>
          ...
          <Section id="chips" title="Chip Specifications">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {CHIPS.map(chip => (
                <div key={chip.label} style={{
                  ...
                  background: "var(--md-sys-color-dark-primary)", border: "1px solid var(--md-sys-color-dark-tertiary)", borderRadius: 10,
                }}>
                  {/* Rendered chip */}
                  <span style={{
                  ...
                  <span style={{ fontSize: 12, color: "var(--md-sys-color-text-muted)", lineHeight: 1.45 }}>{chip.usage}</span>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 14, padding: "12px 16px", borderRadius: 8, background: "rgba(139,146,255,0.06)", border: "1px solid rgba(139,146,255,0.15)" }}>
              <p style={{ fontSize: 12, color: "var(--md-sys-color-text-muted)", lineHeight: 1.6, margin: 0 }}>
                <strong style={{ color: "var(--md-sys-color-text-secondary)" }}>Stacking order on a card:</strong>{" "}
          ...
          <Section id="cards" title="Customer Cards">
            ...
            <div style={{ marginBottom: 32 }}>
              ...
              <p style={{ fontSize: 13, color: "var(--md-sys-color-text-muted)", lineHeight: 1.6, marginBottom: 14 }}>
          ...
          <Section id="search" title="Search States">
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
              ...
              <MockScreen label="Results — mixed types" width={260}>
                <MockHeader title="Customers" />
                <MockSearchBar query="acme" active />
                <div style={{ padding: "2px 0 4px", borderBottom: "1px solid var(--md-sys-color-dark-tertiary)" }}>
                  <div style={{ padding: "4px 14px" }}>
                    <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--md-sys-color-text-muted)" }}>
                      Your Customers · 2
                    </span>
                  </div>
          ...
          <Section id="global" title="Global Search States">
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
              ...
              <MockScreen label="Entry — 'Search all' button" width={260}>
                ...
                <MockEmptyState label='"canyon co" not in your customers' />
                <div style={{ padding: "0 12px 10px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "11px 16px", borderRadius: 999, border: "1px solid var(--md-sys-color-dark-tertiary)" }}>
                    <svg width="14" height="14" viewBox="0 0 18 18" fill="none">
                      <circle cx="9" cy="9" r="7" stroke="var(--md-sys-color-text-secondary)" strokeWidth="1.5"/>
                      <path d="M5.5 9H12.5M9 5.5V12.5" stroke="var(--md-sys-color-text-secondary)" strokeWidth="1.5" strokeLinecap="round"/>
                    </svg>
                    <span style={{ fontSize: 13, fontWeight: 600, color: "var(--md-sys-color-text-primary)" }}>Search all companies</span>
                  </div>
                </div>
              </MockScreen>
              ...
              <MockScreen label="Results — global + assigned" width={260}>
                <MockSearchBar query="canyon co" active pill="ALL" />
                <div style={{ borderBottom: "1px solid var(--md-sys-color-dark-tertiary)" }}>
                  <div style={{ padding: "4px 14px" }}>
                    <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--md-sys-color-text-muted)" }}>All Companies · 2</span>
                  </div>
              ...
              <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                <MockScreen label="In search results" width={340}>
                  ...
                  <div style={{ padding: "4px 0 0" }}>
                    <div style={{ padding: "4px 14px" }}>
                      <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--md-sys-color-text-muted)" }}>All Companies</span>
                    </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                <div key={i} style={{
                  ...
                  background: "var(--md-sys-color-dark-primary)", border: "1px solid var(--md-sys-color-dark-tertiary)", borderRadius: 8,
                }}>
