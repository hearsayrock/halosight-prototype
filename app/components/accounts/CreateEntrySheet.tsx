...
/**
 * FLUTTER HANDOFF: CreateEntrySheet
 * Widget: StatefulWidget (bottom sheet)
 * State: entryType, name, focused, isVisible, duplicates, dupeState, dismissed
 ...
 *         --md-sys-color-neonindigo, --md-sys-color-neonindigo-light, --md-sys-color-warning,
 *         --md-sys-color-warning-light, --md-sys-color-brand-coral,
 *         --radius-xl, --radius-full, --radius-lg
 *
 * Portals into #phone-overlay-root.
 * Accepts initialType ("account" | "lead") — title, CTA, and dupe-check reflect current type.
 * Segmented toggle lets the user switch types without closing.
 * Dupe detection runs only in "lead" mode.
 */
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{ fontSize: 13, fontWeight: 600, color: "var(--md-sys-color-text-primary)", paddingTop: 2 }}
      >
        <Icon name="add" size={14} style={{ color: "var(--md-sys-color-text-primary)" }} />
        Create anyway
      </button>
    </motion.div>
  );
...
export default function CreateEntrySheet({ initialType, initialName = "", onClose, onCreated }: Props) {
  ...
  function handleNameChange(value: string) {
    setName(value);
    setDismissed(false);
    if (entryType === "lead") runDupeCheck(value);
  }

  function handleTypeSwitch(type: EntryType) {
    setEntryType(type);
    setDupeState("idle");
    setDuplicates([]);
    setDismissed(false);
    if (type === "lead" && name.trim().length >= 2) runDupeCheck(name);
  }
  ...
  const showDupeCallout = entryType === "lead" && dupeState === "found" && !dismissed;
          ...
          >
            ...
            <div className="px-5 pt-4 pb-10">
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                >
                  {isLead ? "New lead" : "New account"}
                </h2>
                <button
                  onClick={() => handleTypeSwitch(isLead ? "account" : "lead")}
                ...
                >
                  <Icon name="swap_horiz" size={14} style={{ color: "var(--md-sys-color-text-muted)" }} />
                  <span className="text-sm" style={{ color: "var(--md-sys-color-text-muted)", fontWeight: 600 }}>
                    {isLead ? "Account" : "Lead"}
                  </span>
                </button>
              </div>
              ...
              {/* Duplicate callout (lead mode only) */}
              <AnimatePresence>
                {showDupeCallout && (
                  <DuplicateCallout
                    matches={duplicates}
              ...
              {!showDupeCallout && (
                ...
                >
                  <Icon name="auto_awesome" size={15} style={{ color: "var(--md-sys-color-neonindigo-light)", flexShrink: 0 }} />
                  <span className="text-13" style={{ color: "var(--md-sys-color-neonindigo-light)" }}>
                    Just the name for now — AI fills it in after you visit.
                  </span>
                </div>
              )}
              {/* Create CTA — hidden while dupe callout is showing */}
              {!showDupeCallout && (
                <button
                ...
                >
                  {isLead ? "Create lead" : "Create account"}
                </button>
              )}
